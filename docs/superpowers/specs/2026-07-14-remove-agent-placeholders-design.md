...
将 `backend/app/domains/document/components/vector_store.py` 中的真实实现整体迁入 `backend/app/infrastructure/elasticsearch.py`，包括：

- OpenAI-compatible Embedding model 构造；
...
保留 `backend/app/infrastructure/llm.py`，删除其中对 Agent chat model 的转发，并迁入 `document_worker.py` 与 `celery_worker.py` 重复的：

- `RuntimeImageDescriber`；
- `ChatOpenAI` 图片理解模型构造；
- OpenAI API key/base URL 清洗与校验。

两个 Worker 继续拥有资源生命周期和 cleanup 编排，但共同调用 `app.infrastructure.llm.create_runtime_image_describer` 创建图片描述器。图片描述提示词、模型名 `qwen3.6-flash` 和错误语义保持不变。
...
- `backend/app/infrastructure/llm.py` 中迁移后的图片描述实现
- 所有 Document 专属测试
...
- `app.infrastructure.elasticsearch`、`app.infrastructure.redis` 和 `app.infrastructure.llm` 包含真实实现，不再反向转发 Agent 或 Document 模块。
