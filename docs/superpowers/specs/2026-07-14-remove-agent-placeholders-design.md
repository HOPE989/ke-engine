...
- `backend/app/infrastructure/elasticsearch.py`
- `backend/app/infrastructure/redis.py`

`infrastructure/elasticsearch.py` 仅反向转发 Document 领域内的向量存储实现，仓库没有调用方；真实 Elasticsearch 适配器继续保留在 `domains/document/components/vector_store.py`。`infrastructure/redis.py` 同样没有调用方，且导入了不存在的 `document_vector_storage_lock`，属于失效占位。

`backend/app/infrastructure/redis_lock.py` 是 Document API、Worker、Celery 和 DATA_QUERY 上传流程实际使用的 Redis client 与锁工厂，必须保留。
...
- `backend/app/infrastructure/redis_lock.py` 及其中被 Document 使用的 Redis 锁工厂
- `backend/app/domains/document/components/vector_store.py` 中的真实 Elasticsearch 适配器
- 所有 Document 专属测试
...
2. 删除 Agent/Chat 专属测试、运行代码和无效基础设施转发壳。
...
- 仓库不存在无人使用的 `app.infrastructure.elasticsearch`、`app.infrastructure.llm` 和 `app.infrastructure.redis` 转发壳。
- `app.infrastructure.redis_lock` 与 Document 内的 Elasticsearch 向量存储实现继续存在并通过测试。
