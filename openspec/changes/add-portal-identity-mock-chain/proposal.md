...
当前 Document API 尚未形成统一的请求身份恢复链路，业务代码无法通过稳定方式取得当前用户与租户。现阶段需要先用 Mock 身份走通统一门户鉴权结果进入应用后的消费链路，为后续替换为真实门户 Header 解析建立最小公共骨架。
...
- 在 Document API 中注册公共身份链路，同时允许健康检查绕过身份处理。
...
- `portal-identity-consumption`: 使用默认 Mock 身份模拟统一门户鉴权结果，并通过公共 Middleware、请求级 Principal 和 Dependency 走通 Document API 的身份消费链路。
