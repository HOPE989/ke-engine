...
- 新增最小 LangGraph：`START -> query_rewrite -> END`，用于独立调用、节点测试和后续接入完整 RAG Graph。
- Rewrite 失败时不重试，使用 `original_query` 作为 `standalone_query` 继续完成最小 Graph，并记录可观测的降级状态。
- 新增节点契约测试、Prompt/模型集成测试入口和面向真实模型的 Query Rewrite 评测样例。
- 接入现有 Langfuse callback 传递方式，记录节点、模型调用、实际查询和降级信息。
...
- 影响 `backend/app/domains/rag/`，该目录将新增 Query Rewrite 的模型、服务、Prompt、Graph state/node/builder 和测试边界。
