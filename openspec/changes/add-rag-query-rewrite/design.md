...
本 change 先实现完整 RAG Graph 的第一个纵向增量：调用方提供当前问题、裁剪后的原始会话上下文和可选业务上下文，RAG domain 使用注入的 Chat model 生成一条 `standalone_query`。顶层 `RagState`、`build_rag_graph` 和 Studio 入口始终代表整条 RAG 管线；Query Rewrite 只是当前第一个业务节点，不构成独立子图。
...
本 change 只开发 RAG Graph，不为 Query Rewrite 暴露独立 service。阶段专属的契约、Prompt 和评测辅助代码放在 `graph/query_rewrite/`，模型调用、结构化输出校验和 fallback 放在 `graph/nodes/query_rewrite.py`。该 node 同时提供可直接测试的 `invoke_query_rewrite(...)`，其测试组织参考 Chat `business_understanding` 的方式，但不要求逐项复制其内部结构。
...
### 5. 顶层 RAG Graph 复用现有运行时注入模式

新增管线级 RAG runtime context、`RagState` 和 `build_rag_graph`。builder 支持两种模式：
...
这样当前 RAG Graph 可被直接观察；未来 RAG MCP/应用服务也可以创建更高层的 root trace，并让 Rewrite 自然成为其子运行。fallback 状态和 warning 保留在 Graph 输出中，方便调用方补充业务级属性。
