...
仓库维护一组数据驱动的 Query Rewrite cases，至少覆盖上下文消歧、口语噪声、术语规范化、已独立问题，以及实体、时间、数字、否定、比较和归属约束。样例使用 `required_literals` 等确定性断言验证关键约束保留，而不要求模型输出与某个标准句逐字一致。

真实模型评测由显式命令执行，调用生产 node/Graph 并逐条报告原问题、改写结果、状态和确定性检查。它不由默认 pytest 或 CI 自动触发。
