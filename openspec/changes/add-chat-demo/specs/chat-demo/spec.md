...
The system SHALL expose an anonymous single-turn chat endpoint at `POST /api/v1/chat`.
...
- **THEN** the system SHALL return a request validation error
- **AND** the system MUST NOT call the LLM provider

### Requirement: OpenAI-compatible provider configuration

The system SHALL configure the chat model from OpenAI-compatible environment variables.
...
#### Scenario: Provider returns no answer text

- **WHEN** the configured LangChain chat model returns an empty or missing answer text
- **THEN** the system SHALL return HTTP 502 with an application error explaining that the chat provider returned an empty response
