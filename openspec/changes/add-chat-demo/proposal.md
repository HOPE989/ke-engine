...
- Add focused tests for route mounting, request validation, missing configuration, and successful service behavior with a mocked LLM call.
...
- Configuration: reads `OPENAI_API_KEY`, `OPENAI_BASE_URL`, and `OPENAI_MODEL` from the process environment.
