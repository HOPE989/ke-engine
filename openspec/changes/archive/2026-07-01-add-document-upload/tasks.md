...
- [x] 1.1 RED: add `backend/tests/test_document_config.py` covering
...
- [x] 1.2 RED: run `uv run --extra dev pytest tests/test_document_config.py -q`
  and confirm it fails because document upload settings and env examples are
  absent.
- [x] 1.3 GREEN: add only the required document upload dependencies and settings:
  Alembic, MinIO SDK, Magika, multipart upload support, and MinerU settings.
- [x] 1.4 GREEN: rerun `uv run --extra dev pytest tests/test_document_config.py -q`
  and confirm configuration tests pass.
- [x] 1.5 RED: add `backend/tests/test_document_resource_ownership.py` proving
...
- [x] 1.6 RED: run `uv run --extra dev pytest tests/test_document_resource_ownership.py -q`
  and confirm it fails because the document infrastructure accessors are absent.
- [x] 1.7 GREEN: implement the minimal infrastructure resource ownership needed
  ...
  shutdown hook.
- [x] 1.8 GREEN: rerun `uv run --extra dev pytest tests/test_document_resource_ownership.py -q`
  and confirm resource ownership tests pass.

## 2. Migration and KnowledgeDocument Persistence

- [x] 2.1 RED: add `backend/tests/test_document_migration.py` that inspects the
  ...
  `created_at`.
- [x] 2.2 RED: run `uv run --extra dev pytest tests/test_document_migration.py -q`
  and confirm it fails because Alembic configuration and the migration are
  absent.
- [x] 2.3 GREEN: add Alembic configuration and an initial migration for the exact
  `knowledge_document` schema.
- [x] 2.4 GREEN: rerun `uv run --extra dev pytest tests/test_document_migration.py -q`
  and confirm migration metadata tests pass.
- [x] 2.5 RED: add `backend/tests/test_document_persistence.py` for
...
- [x] 2.6 RED: run `uv run --extra dev pytest tests/test_document_persistence.py -q`
  and confirm it fails because the document model and persistence functions are
  absent.
- [x] 2.7 GREEN: add `models.py` and focused persistence functions in
  `repository.py`; do not introduce a repository class unless a later test
  proves a real lifecycle need.
- [x] 2.8 GREEN: rerun `uv run --extra dev pytest tests/test_document_persistence.py -q`
  and confirm lifecycle and conflict tests pass.

## 3. Upload API Validation and Error Envelope

- [x] 3.1 RED: add `backend/tests/test_document_upload_api_validation.py` for
...
- [x] 3.2 RED: extend the same file for blank `upload_user`, blank
...
- [x] 3.3 RED: add a path-like filename case proving `doc_title` and object-key
  suffixes use a safe basename and raw path segments are not used as object-key
  prefixes.
- [x] 3.4 RED: run `uv run --extra dev pytest tests/test_document_upload_api_validation.py -q`
  and confirm it fails because the document route and validation boundary are
  absent.
- [x] 3.5 GREEN: add `router.py`, `schemas.py`, and route-level validation/error
  mapping. The router must call module-level workflow functions rather than a
  stateless service class.
- [x] 3.6 GREEN: rerun `uv run --extra dev pytest tests/test_document_upload_api_validation.py -q`
  and confirm validation tests pass without MinIO or MinerU calls.

## 4. File Type Detection

- [x] 4.1 RED: add `backend/tests/test_document_file_type.py` covering Magika PDF
  detection, Markdown detection, `.md` / `.markdown` / `.txt` generic text
  fallback, unsupported file type, and Magika runtime failure.
- [x] 4.2 RED: run `uv run --extra dev pytest tests/test_document_file_type.py -q`
  and confirm it fails because detection functions are absent.
- [x] 4.3 GREEN: add `file_types.py` with `detect_document_file_type(...)` and
  the smallest business enum/value shape needed by the workflow.
- [x] 4.4 GREEN: rerun `uv run --extra dev pytest tests/test_document_file_type.py -q`
  and confirm detection tests pass.

## 5. MinIO Storage Helpers

- [x] 5.1 RED: add `backend/tests/test_document_storage.py` covering bucket
...
- [x] 5.2 RED: run `uv run --extra dev pytest tests/test_document_storage.py -q`
  and confirm it fails because the storage helpers are absent.
- [x] 5.3 GREEN: add `storage.py` with document object-key helpers and a thin
  storage adapter or functions around the official MinIO SDK. Keep MinIO SDK
  client ownership in infrastructure.
- [x] 5.4 GREEN: rerun `uv run --extra dev pytest tests/test_document_storage.py -q`
  and confirm storage tests pass with a fake or mocked MinIO client.

## 6. Plain Text Upload Workflow

- [x] 6.1 RED: add `backend/tests/test_document_plain_text_upload.py` covering a
  ...
  document metadata.
- [x] 6.2 RED: extend the same file for MinIO original upload failure returning
  HTTP 502 `document storage failed`, keeping status `INIT`, and not calling any
  conversion function.
- [x] 6.3 RED: run `uv run --extra dev pytest tests/test_document_plain_text_upload.py -q`
  and confirm it fails because `upload_document(...)` orchestration is absent.
- [x] 6.4 GREEN: add `workflow.py` with `upload_document(...)` as a module-level
  async function implementing the plain text path without holding a DB
  transaction across MinIO calls.
- [x] 6.5 GREEN: rerun `uv run --extra dev pytest tests/test_document_plain_text_upload.py -q`
  and confirm the plain text success and storage failure tests pass.

## 7. PDF Conversion Success

- [x] 7.1 RED: add `backend/tests/test_document_pdf_conversion.py` covering
...
- [x] 7.2 RED: add multiple-Markdown selection tests: basename match wins,
  matching parent directory wins second, and lexicographically first normalized
  Markdown path wins last.
- [x] 7.3 RED: add an API-level PDF success test proving the endpoint persists
...
- [x] 7.4 RED: run `uv run --extra dev pytest tests/test_document_pdf_conversion.py -q`
  and confirm it fails because MinerU conversion and PDF workflow functions are
  absent.
- [x] 7.5 GREEN: add `mineru.py`, `markdown.py`, and PDF workflow functions for
  the successful path using local temporary ZIP processing before converted
  object uploads.
- [x] 7.6 GREEN: rerun `uv run --extra dev pytest tests/test_document_pdf_conversion.py -q`
  and confirm PDF success tests pass.

## 8. PDF Failure and Rollback

- [x] 8.1 RED: add `backend/tests/test_document_pdf_failures.py` covering MinerU
  ...
  conflict.
- [x] 8.2 RED: assert each conversion failure after original upload returns HTTP
...
- [x] 8.3 RED: add rollback failure coverage proving conversion failure after
  `CONVERTING` returns HTTP 500 `document state rollback failed` when restoring
  `UPLOADED` fails.
- [x] 8.4 RED: run `uv run --extra dev pytest tests/test_document_pdf_failures.py -q`
  and confirm it fails for the missing failure handling.
- [x] 8.5 GREEN: implement PDF failure normalization, expected-state conflict
  mapping to HTTP 409 `document state conflict`, and rollback failure mapping to
  HTTP 500 `document state rollback failed`.
- [x] 8.6 GREEN: rerun `uv run --extra dev pytest tests/test_document_pdf_failures.py -q`
  and confirm all failure matrix tests pass.

## 9. Contract Verification

- [x] 9.1 Run focused document tests:
...
- [x] 9.2 Run the full backend suite: `uv run --extra dev pytest -q`.
- [x] 9.3 Run migration verification from a clean development database if the
  configured database is available: `uv run alembic downgrade base && uv run alembic upgrade head`.
- [x] 9.4 Run OpenSpec validation: `openspec validate add-document-upload --strict`.
- [x] 9.5 Confirm `openspec status --change add-document-upload` shows the tasks
