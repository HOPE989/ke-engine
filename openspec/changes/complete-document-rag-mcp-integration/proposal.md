...
- Chat 对文档知识类业务意图通过 MCP 调用 RAG，`BUSINESS_DATA_QUERY` 和 `OTHER_BUSINESS` 继续保持当前未接入边界。
- Chat 新增基于证据的回答节点，只使用 MCP 返回的证据生成带引用回答。
...
- `chat-langgraph-runtime`: 将选定的文档知识类 BUSINESS 路径从固定边界节点改为 `knowledge_rag -> grounded_answer`，并通过运行时上下文注入 MCP Client。
- `chat-streaming-completion`: 将 grounded answer 的模型输出投影为现有 `content_delta`，并在成功持久化回答和引用后发送 `completed`。
...
- 修改 Chat Graph state、runtime context、业务路由、completion 事件投影、ASSISTANT 持久化和消息历史契约。
