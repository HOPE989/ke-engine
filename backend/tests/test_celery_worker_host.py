...
def test_celery_worker_exposes_app_with_document_task_include(monkeypatch):
    ...
    calls = []
    missing = object()
    module_name = "app.entrypoints.celery_worker"
    workers_package = importlib.import_module("app.entrypoints")
    ...
    try:
        ...
        assert calls == [["app.domains.document.tasks.vector_storage_compensation"]]
    finally:
